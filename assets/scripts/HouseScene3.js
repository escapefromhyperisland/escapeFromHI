...
createSounds() {
   ...
   windowssound: this.sound.add('windowssound')
    ...
    createBackground(){
        ...
        this.add.sprite(0,0,"house3").setOrigin(0, 0);
    }
    createCards(){
    ...
    update() {
        //MOVEMENT
            ...
            this.fPlayer.anims.play('walk',true);
                ...
                if(this.fPlayer.x>=1190){
                    this.sounds.street.stop();
                    //!launching next scene, pushing Mind parameters
                    this.scene.start('Basement1',{mind:this.mind});  
                }
               
                if(this.fPlayer.x==200){
                   
                    this.createPlayerSub("Basement! I should check it!");
                    this.time.addEvent({ 
                        delay: 1000, 
                        callback: this.hidePlayerSub, 
                        callbackScope: this 
